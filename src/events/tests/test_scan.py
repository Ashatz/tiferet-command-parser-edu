...
def test_extract_text_success(sample_source_file: str) -> None:
    ...
    # Assert imports block, group header, and two event blocks were extracted.
    assert len(result) == 4
    ...
    assert '# *** events' in result[1]['text']
...
def test_extract_text_with_filter(sample_source_file: str) -> None:
    ...
    # Assert imports block, group header, and the filtered block were returned.
    assert len(result) == 3
    assert result[0]['name'] == '__imports__'
    assert result[1]['name'] == '__group_header__'
    assert result[2]['name'] == 'sample_event'
