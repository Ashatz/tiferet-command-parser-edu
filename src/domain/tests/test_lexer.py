...
def test_token_creation_and_access(minimal_token: Token, artifact_token: Token) -> None:
    ...
    assert artifact_token.column == 0
...
def test_token_string_representation(minimal_token: Token) -> None:
    '''
    Token has a useful __str__ / __repr__ for debugging (Pydantic default).
...
def test_token_immutability_after_creation(minimal_token: Token) -> None:
    '''
    Once created, Token fields behave as expected.
    
    Pydantic BaseModel allows mutation unless frozen=True.
    ...
    # Mutation is allowed in non-frozen models
