...
![Tiferet Compiler LR(1) and LALR Automaton](<Tiferet Compiler LR(1)_LALR Automaton.jpg>)
...
Since no states were merged (all 17 LR(1) states have unique cores), the **LALR automaton is identical to the LR(1) automaton**. There are no unique look-ahead states that define new cores — every state's core appears exactly once across the canonical collection — so the LALR and LR(1) constructions yield the same 17-state machine. The 17-state diagram and parse table apply without modification.
